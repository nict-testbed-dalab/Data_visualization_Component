...
/*-----* timeline *-----------------------------------------------------------*/
  ...
  $("#timeline").k2goTimeline(
  {
    ...
    pickMoveEnd    : function(pTimeInfo) {
      ...
      STARScontroller_updateDate(pTimeInfo);    
    },
...
$('#map').on('load', function() {
  //initMap($(this)[0], $Env.webcamApp, $Env.mapApp);
  //$('#map')[0].contentWindow.childFunc();
  //$('#map')[0].contentWindow.testAlert();
  //console.log($('#map')[0].contentWindow.wgapp.val);
  //console.log($('#map')[0].contentWindow.wgapp.map);
  
  //$('#map')[0].contentWindow.$.Map.mapObject.setCenter([135.00130386925252, 34.64972685846791]);
});
